...
class Asserter extends TestResultReporter {
  ...
  isSuccessfulTestResult(actual) {
    if (actual instanceof TestResult) {
      return this.that(actual).isSuccessfulTestResult();
    } else {
      throw new Error('Actual object needs to be a TestResult')
    }
  }

  isFailedTestResultWithMessage(actual, failureMessage) {
    if (actual instanceof TestResult) {
      return this.that(actual).isFailedTestResultWithMessage(failureMessage);
    } else {
      throw new Error('Actual object needs to be a TestResult')
    }
  }
