import { isFunction, isStringWithContent, isUndefined, shuffle } from '../utils/index.js';
...
export class TestSuite {
  ...
  before(initializationBlock) {
    TestSuite.#validateHook(TestSuite.BEFORE_HOOK_NAME, initializationBlock, this.#before);
  ...
  after(releasingBlock) {
    TestSuite.#validateHook(TestSuite.AFTER_HOOK_NAME, releasingBlock, this.#after);
  ...
  #initializeBody(body) {
    TestSuite.#ensureBodyIsValid(body);
    this.#body = body;
  }

  #initializeName(name) {
    TestSuite.#ensureNameIsValid(name);
    this.#name = name;
  }
  ...
  static #ensureNameIsValid(name) {
    if (!isStringWithContent(name)) {
      throw new Error(TestSuite.invalidSuiteNameErrorMessage());
    }
  }

  static #ensureBodyIsValid(body) {
    if (!isFunction(body)) {
      throw new Error(TestSuite.invalidSuiteDefinitionBodyErrorMessage());
    }
  }

  static #validateHook(hookName, hookToSet, existingHook) {
    TestSuite.#validateTheHookIsNotAlreadyDeclared(existingHook, hookName);
    TestSuite.#validateTheHookBlockIsAFunction(hookToSet, hookName);
  }

  static #validateTheHookIsNotAlreadyDeclared(hookBlock, hookName) {
    if (!isUndefined(hookBlock)) {
      throw new Error(TestSuite.alreadyDefinedHookErrorMessage(hookName));
    }
  }

  static #validateTheHookBlockIsAFunction(hookBlock, hookName) {
