...
export class Assertion extends TestResultReporter {
  ...
  isGreaterThan(number) {
    const number1 = Number.parseFloat(this.#actual);
    const number2 = Number.parseFloat(number);
    const result = number1 > number2;
    const failureMessage = () => I18nMessage.of('expectation_be_greater_than', this.#actualResultAsString(), number1.toString(), number2.toString());
    this.#reportAssertionResult(result, failureMessage);
  }
  ...
  isLessThan(number) {
    ...
    const number2 = Number.parseFloat(number);
    const result = number1 < number2;
    const failureMessage = () => I18nMessage.of('expectation_be_less_than', this.#actualResultAsString(), number1.toString(), number2.toString());
    this.#reportAssertionResult(result, failureMessage);
  }
