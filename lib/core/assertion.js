...
export class Assertion extends TestResultReporter {
  ...
  includes(expectedObject, equalityCriteria) {
    const resultIsSuccessful = convertToArray(this.#actual).find(element =>
      Assertion.#areConsideredEqual(element, expectedObject, equalityCriteria));
  ...
  isIncludedIn(expectedCollection, equalityCriteria) {
    const resultIsSuccessful = expectedCollection.find(element =>
      Assertion.#areConsideredEqual(element, this.#actual, equalityCriteria));
  ...
  includesExactly(...objects) {
    const resultIsSuccessful = Assertion.#haveElementsConsideredEqual(this.#actual, objects);
  ...
  raises(errorExpectation) {
    ...
    try {
      ...
      const assertionPassed = Assertion.#checkIfErrorMatchesExpectation(errorExpectation, actualError);
      const errorMessage = () =>
        I18nMessage.of(
  ...
  doesNotRaise(notExpectedError) {
    ...
    try {
      ...
      const errorCheck = Assertion.#checkIfErrorMatchesExpectation(notExpectedError, actualError);
  ...
  static #areConsideredEqual(objectOne, objectTwo, equalityCriteria) {
    return EqualityAssertionStrategy.evaluate(objectOne, objectTwo, equalityCriteria).comparisonResult;
  }

  static #checkIfErrorMatchesExpectation(errorExpectation, actualError) {
    if (isRegex(errorExpectation)) {
      return errorExpectation.test(actualError);
    }
    return Assertion.#areConsideredEqual(actualError, errorExpectation);
  }
  ...
  static #haveElementsConsideredEqual(collectionOne, collectionTwo) {
    ...
    for (let index = 0; index < collectionOne.length; index += 1) {
      const includedInOne = collectionOne.find(element =>
        Assertion.#areConsideredEqual(element, collectionTwoArray[index]));
      const includedInTwo = collectionTwo.find(element =>
        Assertion.#areConsideredEqual(element, collectionOneArray[index]));
  ...
  #validateStringMatch() {
    if (!isString(this.#actual)) {
      throw new InvalidAssertionError(I18nMessage.of('invalid_actual_object_in_string_match', this.#actualResultAsString()));
    }
  }
}
