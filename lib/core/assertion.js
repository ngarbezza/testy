...
export class Assertion extends TestResultReporter {
  ...
  isSuccessfulTestResult() {
    const isSuccessfulTestResult = this.#actual.isSuccess()
    this.#reportAssertionResult(
        isSuccessfulTestResult,
        () => this.#actual.failureMessage())
  }

  isFailedTestResultWithMessage(failureMessage) {
    const isFailedTestResult = this.#actual.isFailure()
    this.#reportAssertionResult(
        isFailedTestResult,
        () => I18nMessage.of('expectation_failure_to_happen', failureMessage.expressedIn(I18n.default())))
  }
