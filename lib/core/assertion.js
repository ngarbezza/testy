...
export class Assertion extends TestResultReporter {
  ...
  raises(errorExpectation) {
    ...
    try {
      this._evaluateActualObjectAsFunction();
  ...
  _ensureActualObjectIsAFunction() {
    if (!isFunction(this._actual)) {
      throw new InvalidAssertionError(I18nMessage.of('invalid_actual_object_in_exception_assertion', this._actualResultAsString()));
    }
  }

  _evaluateActualObjectAsFunction() {
    this._actual.call();
  }
}
