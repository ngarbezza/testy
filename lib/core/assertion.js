...
export class Assertion extends TestResultReporter {
  ...
  isNearTo(number, precisionDigits) {
    const precisionDigitsOrDefault = precisionDigits ?? 4
    const result = asFloat(this.#actual.toFixed(precisionDigitsOrDefault)) === number;
    const failureMessage = () => I18nMessage.of('expectation_be_near_to', this.#actualResultAsString(), number.toString(), precisionDigitsOrDefault.toString());
