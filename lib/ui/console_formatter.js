...
export class ConsoleFormatter extends Formatter {
  ...
  displayError(message) {
    this.print(`${this.#withColor(this.internationalized(message), red)}`);
  }
  ...
  #displayConfigurationSummary(paths, configuration) {
    ...
    this.print(this.#inBold(this.translated('starting_testy')));
  ...
  #displayResult(result, test, color) {
    this.print(`[${color}${this.#inBold(result)}] ${this.#withColor(test.name(), color)}`);
  }
