...
class I18n {
  ...
  constructor(languageCode, translations = TRANSLATIONS) {
    I18n.#assertLanguageIsSupported(languageCode, translations);
  ...
  translate(key, ...params) {
    ...
    return I18n.#evaluateWithPotentialArguments(translatedText, params);
  }

  // assertions

  static #assertLanguageIsSupported(languageCode, translations) {
  ...
  static #validateNumberOfArgumentsMatch(expectedParams, givenParams) {
    if (expectedParams.length !== givenParams.length) {
      throw new Error(I18n.#wrongNumberOfArgumentsErrorMessage(expectedParams, givenParams));
    }
  }

  // private

  static #evaluateWithPotentialArguments(translatedText, params) {
    const translatedTextArguments = translatedText.match(/%s/g) || [];
    I18n.#validateNumberOfArgumentsMatch(translatedTextArguments, params);
  ...
  // error messages
  static #wrongNumberOfArgumentsErrorMessage(expectedParams, givenParams) {
