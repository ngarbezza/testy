  ...
  async #loadFileHandlingErrors(filePath) {
    ...
    try {
      ...
      if (fileExtension === '.ts') {
        ...
        importPath = transpilationResult.importPath
        isTempFileCreated = transpilationResult.isTempFileCreated
      }
  ...
  #cleanupTemporaryFile(shouldRemoveFile, path) {
    if (shouldRemoveFile && path && fs.existsSync(path)) {
      try {
        ...
        console.error(`[Testy] Warning: Failed to delete temporary file ${path}:`, cleanupError);
      }
    }
  }
