...
class Testy {
  ...
  async #loadFileHandlingErrors(file) {
    ...
    try {
      ...
      if (ext === '.ts') {
        ...
        tempFilePath = path.join(tempDir, `${path.basename(file)}.js`);
        fs.writeFileSync(tempFilePath, jsCode);
        isTempFileCreated = true;
        importPath = tempFilePath;
      }
      // Note: We let `import` handle .js files directly without an explicit check.

      // Convert the path to a file URL for robust importing.
      ...
      if (isTempFileCreated && tempFilePath && fs.existsSync(tempFilePath)) {
        try {
          fs.unlinkSync(tempFilePath);
        } catch (cleanupError) {
          console.error(`[Testy] Warning: Failed to delete temporary file ${tempFilePath}:`, cleanupError);
        }
      }
    }
  }
  ...
  #transpileTypeScript(typescriptCode) {
    ...
    const result = ts.transpileModule(typescriptCode, compilerOptions);
