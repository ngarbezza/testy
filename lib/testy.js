...
class Testy {
  ...
  async #loadAllFilesIn(path) {
    ...
    for (const file of allFilesMatching(path, this.#testFilesFilter())) {
      // eslint-disable-next-line no-await-in-loop
      await Testy.#loadFileHandlingErrors(file);
    }
  }

  static async #loadFileHandlingErrors(filePath) {
    ...
    try {
      ...
      Testy.#cleanupTemporaryFile(isTempFileCreated, importPath);
    }
  }

  static #cleanupTemporaryFile(shouldRemoveFile, path) {
