...
const RunTests = {
  ...
  async execute(params) {
    try {
      const sentConfigurationParams = params.slice(1)
      const selectedConfiguration = ParametersParser.generateRunConfigurationFromParams(sentConfigurationParams)
      const testyInstance = Testy.configuredWith(Configuration.withConfiguration(selectedConfiguration));
      await testyInstance.run(params.slice(0,1));
      this._exitSuccessfully();
    } catch (error) {
