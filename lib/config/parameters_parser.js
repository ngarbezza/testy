...
export class ParametersParser {
  ...
  static sanitizeParameters(params) {
    const languageParamIndex = params.findIndex(p => p === '-l' || p === '--language')
...
class InvalidConfigurationParameter {
  ...
  static handle(configurationParam) {
    throw Error(`Cannot parse invalid run configuration parameter ${configurationParam}. Please run --help option to check available options.`)
  }
}