...
export class ParametersParser {
  ...
  static generateRunConfigurationParameter(param) {
    const paramParser = [
      ...
      OutputConfigurationParameterParser,
  ...
  static assertParamIsNotPathParam(param, previousParam) {
    const previousParamTakesAValue =
      [...DIRECTORY_IDENTIFIERS, ...EXTENSION_IDENTIFIERS, ...OUTPUT_IDENTIFIERS].includes(previousParam);

    if (!this.isRawConfigurationParam(param) && !previousParamTakesAValue) {
...
class OutputConfigurationParameterParser {
  ...
  static handle(consoleParam) {
    const [, value] = consoleParam.split(' ');
