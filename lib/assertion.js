...
class Assertion extends TestResultReporter {
  ...
  includes(expectedObject, equalityCriteria) {
    ...
    const failureMessage = new InternationalizedMessage('expectation_include', this._actualResultAsString(), prettyPrint(expectedObject));
  ...
  isIncludedIn(expectedCollection, equalityCriteria) {
    ...
    const failureMessage = new InternationalizedMessage('expectation_be_included_in', this._actualResultAsString(), prettyPrint(expectedCollection));
    this._reportAssertionResult(resultIsSuccessful, failureMessage);
  }
  ...
  doesNotInclude(expectedObject, equalityCriteria) {
    const resultIsSuccessful = !convertToArray(this._actual).find(element =>
      this._areConsideredEqual(element, expectedObject, equalityCriteria));
    const failureMessage = new InternationalizedMessage('expectation_not_include', this._actualResultAsString(), prettyPrint(expectedObject));
    this._reportAssertionResult(resultIsSuccessful, failureMessage);
  }
  ...
  isNotIncludedIn(expectedCollection, equalityCriteria) {
    const resultIsSuccessful = !expectedCollection.find(element =>
      this._areConsideredEqual(element, this._actual, equalityCriteria));
    const failureMessage = new InternationalizedMessage('expectation_be_not_included_in', this._actualResultAsString(), prettyPrint(expectedCollection));
    this._reportAssertionResult(resultIsSuccessful, failureMessage);
  }
  
  includesExactly(...objects) {
    const resultIsSuccessful = this._haveElementsConsideredEqual(this._actual, objects);
    const failureMessage = new InternationalizedMessage('expectation_include_exactly', this._actualResultAsString(), prettyPrint(objects));
    this._reportAssertionResult(resultIsSuccessful, failureMessage);
  }
  
  isEmpty() {
    const resultIsSuccessful = numberOfElements(this._actual || {}) === 0 && notNullOrUndefined(this._actual);
    const failureMessage = new InternationalizedMessage('expectation_be_empty', this._actualResultAsString());
    this._reportAssertionResult(resultIsSuccessful, failureMessage);
  }
  ...
  isNotEmpty() {
    ...
    const failureMessage = new InternationalizedMessage('expectation_be_not_empty', this._actualResultAsString());
    this._reportAssertionResult(resultIsSuccessful, failureMessage);
  }
  ...
  isNearTo(number, precisionDigits = 4) {
    ...
    const failureMessage = new InternationalizedMessage('expectation_be_near_to', this._actualResultAsString(), number.toString(), precisionDigits.toString());
  ...
  matches(regex) {
    ...
    const failureMessage = new InternationalizedMessage('expectation_match_regex', this._actualResultAsString(), regex);
    this._reportAssertionResult(result, failureMessage, false);
  }
