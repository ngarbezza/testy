...
class Assertion extends TestResultReporter {
  ...
  _identityAssertion(expected, shouldBeIdentical) {
    ...
    const resultIsSuccessfulWithUndefinedChecked = isUndefined(comparisonResult) ? false : resultIsSuccessful;
    
    if (overrideFailureMessage) {
      this._reportAssertionResult(resultIsSuccessfulWithUndefinedChecked, overrideFailureMessage);
    } else {
      ...
      this._reportAssertionResult(resultIsSuccessfulWithUndefinedChecked, expectationMessage);
    }
  }
