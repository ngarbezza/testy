...
const { prettyPrint, isUndefined, isRegex, isString, notNullOrUndefined, numberOfElements, convertToArray } = require('./utils');
...
class Assertion extends TestResultReporter {
  ...
  includes(expectedObject, equalityCriteria) {
    ...
    const failureMessage = I18n.message('expectation_include', this._actualResultAsString(), prettyPrint(expectedObject));
  ...
  isIncludedIn(expectedCollection, equalityCriteria) {
    ...
    const failureMessage = I18n.message('expectation_be_included_in', this._actualResultAsString(), prettyPrint(expectedCollection));
    this._reportAssertionResult(resultIsSuccessful, failureMessage);
  }
  ...
  doesNotInclude(expectedObject, equalityCriteria) {
    const resultIsSuccessful = !convertToArray(this._actual).find(element =>
      this._areConsideredEqual(element, expectedObject, equalityCriteria));
    const failureMessage = I18n.message('expectation_not_include', this._actualResultAsString(), prettyPrint(expectedObject));
    this._reportAssertionResult(resultIsSuccessful, failureMessage);
  }
  ...
  isNotIncludedIn(expectedCollection, equalityCriteria) {
    const resultIsSuccessful = !expectedCollection.find(element =>
      this._areConsideredEqual(element, this._actual, equalityCriteria));
    const failureMessage = I18n.message('expectation_be_not_included_in', this._actualResultAsString(), prettyPrint(expectedCollection));
    this._reportAssertionResult(resultIsSuccessful, failureMessage);
  }
  
  includesExactly(...objects) {
    const resultIsSuccessful = this._haveElementsConsideredEqual(this._actual, objects);
    const failureMessage = I18n.message('expectation_include_exactly', this._actualResultAsString(), prettyPrint(objects));
    this._reportAssertionResult(resultIsSuccessful, failureMessage);
  }
  ...
  doesNotRaiseAnyErrors() {
    try {
      this._actual.call();
      this._reportAssertionResult(true);
    } catch (error) {
      this._reportAssertionResult(false, I18n.message('expectation_no_errors', prettyPrint(error)));
    }
  }
  ...
  isNearTo(number, precisionDigits = 4) {
    ...
    const failureMessage = I18n.message('expectation_be_near_to', this._actualResultAsString(), number.toString(), precisionDigits.toString());
  ...
  matches(regex) {
    ...
    const failureMessage = I18n.message('expectation_match_regex', this._actualResultAsString(), regex);
    this._reportAssertionResult(result, failureMessage);
  }
  ...
  _exceptionAssertion(errorExpectation, shouldFail) {
    let assertionHasFailed = false;
    let actualError;
    try {
      this._actual.call();
      assertionHasFailed = !shouldFail;
    } catch (error) {
      actualError = error;
      ...
      if (isUndefined(actualError)) {
        failureMessage = I18n.message('expectation_error', prettyPrint(errorExpectation));
      } else {
        if (shouldFail) {
          failureMessage = I18n.message('expectation_different_error', prettyPrint(errorExpectation), prettyPrint(actualError));
        } else {
          failureMessage = I18n.message('expectation_no_error', prettyPrint(actualError));
        }
      }
      this._reportAssertionResult(assertionHasFailed, failureMessage, true);
    }
  }
  ...
  _reportAssertionResult(wasSuccess, matcherFailureMessage, overrideFailureMessage) {
    if (wasSuccess) {
      ...
      if (isString(matcherFailureMessage)) {
