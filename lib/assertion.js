...
const { I18n, I18nMessage } = require('./i18n');
...
class Assertion extends TestResultReporter {
  ...
  doesNotRaiseAnyErrors() {
    try {
      ...
      this._reportAssertionResult(false, I18nMessage.of('expectation_no_errors', prettyPrint(error)));
    }
  }
  ...
  _equalityAssertion(expected, criteria, shouldBeEqual) {
    ...
    if (overrideFailureMessage) {
      ...
      const finalMessage = I18nMessage.joined([expectationMessage, additionalFailureMessage], ' ');
  ...
  _exceptionAssertion(errorExpectation, shouldFail) {
    ...
    try {
      ...
      if (isUndefined(actualError)) {
        failureMessage = I18nMessage.of('expectation_error', prettyPrint(errorExpectation));
      } else {
        if (shouldFail) {
          failureMessage = I18nMessage.of('expectation_different_error', prettyPrint(errorExpectation), prettyPrint(actualError));
        } else {
          failureMessage = I18nMessage.of('expectation_no_error', prettyPrint(actualError));
        }
      }
      this._reportAssertionResult(assertionHasFailed, failureMessage);
    }
  }
  ...
  _reportAssertionResult(wasSuccess, matcherFailureMessage) {
    if (wasSuccess) {
      this.report(TestResult.success());
    } else {
      this.report(TestResult.failure(matcherFailureMessage));
    }
  }
