...
class TestSuite {
  ...
  run(failFastMode = FailFast.default()) {
    ...
    return this.runTests(failFastMode)
      .then(() => this._callbacks.onFinish(this));
  }
  ...
  evaluateBeforeBlock() {
    return Promise.resolve(this._before && this._before.call());
  }
  
  runTests(failFastMode) {
    return this.tests()
      .reduce((lastTestRun, test) =>
        lastTestRun.then(() => {
          this._currentTest = test;
          return this.evaluateBeforeBlock().then(() => test.run(failFastMode));
        }),
        Promise.resolve()
      );
  }
